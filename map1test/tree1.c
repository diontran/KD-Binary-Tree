...
node_t *nearestNode(double xinp, double yinp, node_t *par, double sD) {
    ...
    // if keyd is greater than shortest distance proceed down either left or right branch
        ...
        if(result1<0){
            if(curr->left != NULL){
                return nearestNode(xinp, yinp, curr->left, sD);
            }
        }
        // case 2: key point is larger
        else if(result1>0){
            if(curr->right != NULL){
                return nearestNode(xinp, yinp, curr->right, sD);
            }
        }
    ...
    // if keyd is less than or equal to shortest distance, proceed down both branches
        ...
        if(curr->right != NULL && curr->left != NULL){
            ...
            return nearestNode(xinp, yinp, par->left, sD);
        }
        else if(curr->right != NULL && curr->left == NULL){
            return nearestNode(xinp, yinp, par->right, sD);
        }
    }
