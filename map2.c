...
int main(int argc, char **argv){
    ...
    printf("**********THIS IS A MAP2 CLOSEST NODE**********\n");
    if (argc < 2){
        return 0;
    } 
...
//     printLeaf(newTree->root->right->left, stdout);
    ...
    while(getline(&buffer, &bufferSize, stdin) > 0){
        ...
        printNearestNode(xinp, yinp, rad, newTree->root, sD, count, output);
        if(sD>rad){
            printf("%lf %lf %lf --> NOTFOUND\n", xinp, yinp, rad);
        }
