...
void printNearestNode(double xinp, double yinp, double rad, node_t *par, double sD, FILE *file) {
    ...
    if(keyD > sD){
        ...
        if(result1<0){
            if(curr->left != NULL){
                printNearestNode(xinp, yinp, rad, curr->left, sD, file);
            }
        }
        // case 2: key point is larger
        else if(result1>0){
            if(curr->right != NULL){
                printNearestNode(xinp, yinp, rad, curr->right, sD, file);
            }
        }
